...
class DeepResearchLogicalCoherenceRelevanceModel(BaseRewardModel):
    @property
    def name(self) -> str:
        return RewardModelType.deep_research_logical_coherence_relevance.value

    def __init__(self, device: str, scoring_type: None):
        super().__init__()
        self.device = device
        self.scoring_type = scoring_type
        self.relevance_prompt = DeepResearchLogicalCoherenceRelevancePrompt()
    ...
    async def check_response(self, synapse: DeepResearchSynapse) -> float:
        try:
            response = await self.relevance_prompt.get_response(
                synapse.report.__str__()
            )

            return self.relevance_prompt.extract_score(response) / 10

        except Exception as e:
            bt.logging.error(f"check_logical_coherence of report error: {str(e)}")
            return 0.0
    ...
    async def get_rewards(
        ...
        try:
            ...
            for response, uid_tensor in zip(responses, uids):
                ...
                bt.logging.info(
                    f"UID {uid}: deep research logical coherence relevance score => {final_score}"
                )
            ...
            bt.logging.info(
                f"========== Deep Research Logical Coherence Check Zero Scores ({len(zero_scores)} cases) =========="
            )
            bt.logging.info(json.dumps(zero_scores))
            bt.logging.info(
                f"======== Deep Research Logical Coherence Check Non-Zero Scores ({len(non_zero_scores)} cases) ========"
            )
