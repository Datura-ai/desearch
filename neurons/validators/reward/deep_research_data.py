...
class DeepResearchDataRelevanceModel(BaseRewardModel):
    ...
    async def check_section_data(self, section: ReportItem) -> float:
        try:
            results = await self.web_scraper_actor.scrape_metadata(section.links)
    ...
    async def check_response(self, synapse: DeepResearchSynapse) -> float:
        try:
            sections = random.choices(synapse.report, k=RANDOM_SECTIONS_COUNT)

            scores = await collect_responses(
                [self.check_section_data(section) for section in sections]
            )

            return sum(scores) / len(scores) if scores else 0.0

        except Exception as e:
            bt.logging.error(f"deep_research_data check_response error: {str(e)}")
            return 0.0
